...
def run_game(filename, start=None, end=None):
    ...
    if start is None:
        while True:
            ...
            if (a, b) != (c, d) and not maze.is_wall(a, b) and not maze.is_wall(c, d) and \
                    len(dfs_solver.solve_maze(maze, (a, b), (c, d))):
                start = (a, b)
                end = (c, d)
                break
    ...
    while not done:
        ...
        if current == end:
            ...
            messagebox.showwarning('', 'Maze solved in {}. Congratulations Maze Runner!'.format(
            ...
            while True:
                ...
                if not maze.is_wall(x, y) and len(dfs_solver.solve_maze(maze, current, (x, y))):
                    end = (x, y)
                    break
        ...
        if keys[pygame.K_t]:
            # print("solution")
            path = dfs_solver.solve_maze(maze, current, end)
